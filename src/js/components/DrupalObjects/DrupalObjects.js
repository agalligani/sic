...
//   <div>
//     <p>
//       Node creation and deletion works only if are logged into Drupal as a user
...
class NodeList extends React.Component {
  ...
  async refresh() {
    ...
    try {
      ...
      const response = await axios.get("/node/rest?_format=json"); // wait for the POST AJAX request to complete
      if (response.data) {
          ...
          {this.state.nodes.map((node, index) => {
            ...
            return (
              <tr key={index}>
                ...
                <td>
                  <button onClick={e => deleteNode(node.nid)}>x</button>
                </td>
              </tr>
            );
          })}
